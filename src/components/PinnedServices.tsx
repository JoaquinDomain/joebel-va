      ...
      <div ref={panel} className="self-start md:h-[70vh]">
        <div className="relative h-72 overflow-hidden border border-[#e6dbcb] md:h-full">
          ...
          <div className="absolute bottom-0 left-0 p-8">
            <p className="text-[11px] uppercase tracking-[0.3em] opacity-70">Your partner</p>
      ...
      <div className="space-y-10">
        {services.map((s, i) => (
          <article key={s.title} className="service-card border border-[#e6dbcb] bg-white/50 p-8">
            ...
            <p className="mt-3 text-sm leading-relaxed opacity-80">{s.blurb}</p>
            <ul className="mt-6 grid gap-3 text-sm opacity-80 sm:grid-cols-2">
              {s.items.map((item) => (
